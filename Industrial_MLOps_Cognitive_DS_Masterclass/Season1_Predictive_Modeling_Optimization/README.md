...
각 파일 링크를 클릭하여 해당하는 주피터 노트북 실습 템플릿으로 바로 이동할 수 있습니다.

### 🧱 [Phase 1: Industrial Time-Series Preprocessing](./notebook/01_timeseries_preprocessing.ipynb)
센서 노이즈 필터링, 결측치 및 이상치 처리 기법을 배웁니다.
*   **실습 내용**: Rolling window 기법을 활용한 시간 스케일 변환, 칼만 필터(Kalman Filter) 및 이동평균을 통한 센서 노이즈 필터링 기법 구현.

### 🕸️ [Phase 2: Imbalanced Data & Feature Engineering](./notebook/02_imbalance_and_features.ipynb)
...
### 🧠 [Phase 3: Equipment Failure Classification](./notebook/03_failure_classification.ipynb)
...
### 🏢 [Phase 4: RUL (Remaining Useful Life) Regression](./notebook/04_rul_regression.ipynb)
설비가 고장나기 전까지 남은 가동 가능 시간(RUL)을 정교하게 예측합니다.
*   **실습 내용**: 시계열 회귀 모델링, RMSE 및 MAE 성능 평가 지표 최적화, 예측 추세선 가시화.

### ⚙️ [Phase 5: Explainable AI & Root Cause Analysis](./notebook/05_explainable_ai_rca.ipynb)
